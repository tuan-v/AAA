<template>
  <header
    ...
    <div class="flex items-center justify-between w-full px-3 py-3 lg:px-6 lg:py-4">
      ...
      <nav class="hidden lg:flex items-center gap-1">
        <a v-for="item in navItems" :key="item.name" target="_blank"
          :href="item.name === 'Trang chủ' || hasCompanies ? item?.href : 'javascript:void(0)'"
    ...
    <div v-if="isMobileNavOpen" class="lg:hidden w-full px-3 pb-3 border-t border-gray-200 dark:border-gray-800">
      <nav class="flex flex-col gap-1 pt-2">
        <a v-for="item in navItems" :key="item.name"
          :href="item.name === 'Trang chủ' || hasCompanies ? item.href : 'javascript:void(0)'"
          @click="handleNavClick(item)"
          class="px-4 py-2.5 text-sm font-medium rounded-lg transition-colors duration-200 cursor-pointer" :class="[
            activeNav === item.name
              ? 'text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-900/20'
              : 'text-gray-700 hover:text-gray-900 hover:bg-gray-100 dark:text-gray-300 dark:hover:text-white dark:hover:bg-gray-800'
          ]">
...
const handleNavClick = (item) => {
  ...
  if (!hasCompanies.value) {
    toast.error("Bạn chưa có công ty quản lý. Vui lòng tạo công ty trước khi sử dụng tính năng này.", {
      position: "top-right",
      autoClose: 5000,
