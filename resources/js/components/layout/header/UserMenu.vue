<template>
    ...
    <button @click.prevent="toggleDropdown" class="relative flex items-center gap-2 p-2 pr-3.5 rounded-xl transition-all duration-200 
      ...
      <div class="hidden sm:flex flex-col items-start flex-1 min-w-0">
        ...
        <span class="hidden md:block text-xs text-gray-500 dark:text-gray-400 leading-tight truncate 
          ...
          {{ user?.company?.name || 'Chưa có công ty' }}
        </span>
      </div>
  ...
  <Teleport to="body">
    <div v-if="modalOpen" class="fixed inset-0 z-[99999] flex items-center justify-center overflow-y-auto">
      <div class="fixed inset-0 bg-black/70 z-[99999]" @click="closeModal"></div>

      <div class="relative z-[100000] w-full max-w-md mx-4 my-8">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700">
          <!-- Header -->
          <div class="sticky top-0 z-10 p-5 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
          ...
          <div class="max-h-[60vh] overflow-y-auto asfy-modal-scroll">
            <div v-if="companies.length === 0" class="py-8 text-center">
              <div
              ...
              <p class="text-gray-700 dark:text-gray-300 font-medium mb-1">Không có công ty nào</p>
              <p class="text-sm text-gray-500 dark:text-gray-400">Thêm công ty để bắt đầu sử dụng</p>
            </div>

            <div v-else class="p-4 space-y-2">
              <div v-for="company in companies" :key="company.id"
                @click="company.id !== user?.company_id && viewCompany(company)" :class="[
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between mb-1">
                      ...
                      <span v-if="company.id === user?.company_id"
                  ...
                  <svg v-if="company.id !== user?.company_id"
                    class="w-4 h-4 text-gray-400 group-hover:text-blue-500 transition-colors flex-shrink-0" fill="none"
          ...
          <!-- Footer -->
          <div class="sticky bottom-0 p-4 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
              <div class="text-sm text-gray-600 dark:text-gray-400">
                Hiển thị <span class="font-semibold text-gray-800 dark:text-gray-200">{{ companies.length }}</span> công
                ty
              </div>
              <button @click="closeModal"
      ...
      <div class="flex flex-col items-center gap-8">
        <div class="relative w-40 h-40 md:w-48 md:h-48">
          ...
          <div class="absolute inset-0 flex items-center justify-center">
            <img src="/resource/asfy-images/asfy-logo.png" alt="ASFY Logo"
...
const quickSwitchCompanies = computed(() => {
  return companies.value.filter(company => company.id !== user.value?.company_id).slice(0, 3)
})
...
const getCurrentCompanyEmail = () => {
  ...
  if (user.value?.company_id) {
    const currentCompany = companies.value.find(c => c.id === user.value.company_id)
