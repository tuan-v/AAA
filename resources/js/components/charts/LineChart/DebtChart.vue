    ...
    <div class="w-full h-full">
        ...
        <div v-else class="flex items-center justify-center h-full">
            <div class="text-center">
                ...
                <p class="text-gray-500 dark:text-gray-400 font-medium">Không có dữ liệu công nợ</p>
                <p class="text-sm text-gray-400 dark:text-gray-500 mt-2">Chưa có giao dịch nào được ghi nhận</p>
            </div>
        </div>
    </div>
</template>
...
const hasData = computed(() => {
    return props.series && props.series.length > 0 && props.series[0]?.data?.length > 0
})
...
const chartSeries = computed(() => {
    ...
    // Transform series: chuyển [timestamp, value] thành [index, value]
    return props.series.map(serie => {
        return {
            name: serie.name,
            data: serie.data.map(point => {
                const index = uniqueTimestamps.indexOf(point[0])
...
// Computed để lưu mapping timestamp -> label
...
// Computed để lưu mapping index -> timestamp cho tooltip
const indexToTimestamp = computed(() => {
    const allTimestamps = props.series.flatMap(s => s.data.map(d => d[0]))
...
// Hàm tính nice number - làm tròn số về dạng đẹp
...
function calculateYAxis(data) {
    ...
    return {
        min: niceMin,
        max: niceMax,
        tickAmount: Math.min(Math.max(tickAmount, 5), 10)
    }
}
...
const chartOptions = computed(() => {
    return {
        ...
        xaxis: {
            type: 'numeric',
            tickAmount: xAxisLabels.value.length - 1,
            labels: {
                rotate: -45,
                rotateAlways: true,
                trim: false,
        ...
        tooltip: {
            ...
            custom: function ({ series, seriesIndex, dataPointIndex, w }) {
                ...
                const timestamp = indexToTimestamp.value[dataPointIndex]
                const date = new Date(timestamp)
                const d = date.getDate().toString().padStart(2, '0')
                const m = (date.getMonth() + 1).toString().padStart(2, '0')
...
/* Dark mode - không cần CSS vì dùng custom tooltip HTML */
</style>