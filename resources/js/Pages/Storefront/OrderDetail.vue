<template>
    ...
    <div class="min-h-screen bg-[#f4f2ed] text-[#111]">
        ...
        <main class="mx-auto max-w-[1200px] px-5 py-10 lg:px-9 lg:py-14">
            <div v-if="notice" class="mb-6 rounded-xl bg-emerald-100 px-5 py-4 text-sm font-bold text-emerald-900">{{ notice }}</div>
            <div v-if="error" class="mb-6 rounded-xl bg-red-100 px-5 py-4 text-sm font-bold text-red-800">{{ error }}</div>
            ...
            <div class="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
                ...
                <aside class="h-fit border border-black/15 bg-white/70 p-6 lg:sticky lg:top-6">
                    ...
                    <button v-if="order.cancelable" :disabled="cancelling" class="mt-6 w-full rounded-full border border-red-700 px-5 py-3 text-sm font-black text-red-700 hover:bg-red-700 hover:text-white disabled:opacity-50" @click="cancelOrder">
...
async function cancelOrder() {
    ...
    try {
        ...
        error.value = Object.values(e.response?.data?.errors || {}).flat()[0] || e.response?.data?.message || 'Không thể hủy đơn hàng.';
    } finally {
