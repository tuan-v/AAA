    ...
    <AdminLayout>
        <PageBreadcrumb
            ...
            :items="[
                ...
                { text: 'Lịch sử giao dịch', link: null },
            ]"
        />
        <div class="mb-5">
            <h2 class="text-2xl font-bold text-gray-900">Lịch sử giao dịch</h2>
            <p class="mt-1 text-sm text-gray-500">
                Theo dõi toàn bộ biến động thu, chi và số dư theo từng tài
                khoản.
            </p>
        </div>
        <div
        ...
        <DataTable
            ...
            emptyMessage="Chưa có lịch sử giao dịch"
        />
        <Pagination
...
import { formatMoney, formatVietnamDateTime } from "@/config/helpers";
...
const columns = [
    {
        ...
        align: "text-left",
        render: (row) => h("span", {}, formatVietnamDateTime(row.created_at)),
    },
    {
        label: "Mã giao dịch",
        align: "text-left",
    ...
    },
    {
        ...
        render: (row) =>
            h("div", { class: "flex flex-col" }, [
                h("span", { class: "font-medium" }, row.account?.name ?? "-"),
                h(
                    "span",
...
async function fetchData(page = 1) {
    const { data } = await axios.get("/api/accountant/account-ledgers", {
...
onMounted(async () => {
    ...
    filters.value[1].options = accounts.map((account) => ({
        value: account.id,
